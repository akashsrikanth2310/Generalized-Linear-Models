...
train = within(train, rm("Category", "currency","Duration","endDay"))      #choosing the columns given in problem description for dummy values

#generating the linear regression model

fit.all <- glm(`Competitive?` ~ ., family=binomial,data=train)
coefs <- coefficients(fit.all)                                             #select all coefficients in the glm

print(summary(fit.all))                                                    #prints fit.all summary stats 
...
subset = c("Competitive?", names(coefs)[counter])         #creating subset for fit.one values

#fit.single hold values of glm with the highest regression coefficient

fit.single = glm(`Competitive?` ~ ., family=binomial,data=train[subset])    #logistic regression for fit.one

print(fit.single)                                                           #printing fit.one stats
...
iterator <- 1
lister <- ("Competitive?")                                        #creating list to hold all coefficient names that have Pr(>|z|) is less 0.05 in summary.fit
while(iterator < 16)
{
...
lister = lister[-2]                                            #used to remove intercept value obtained
lister = lister[-5]
...
fit.reduced = glm(`Competitive?` ~., family=binomial, data=train[lister])  # the reduced model

print(fit.reduced)                                              #prints reduced model stats
